...
// NhConversion.cpp
//
#include <windows.h>	// WideCharToMultiByte, LPSTRìô
#include <assert.h>
...
LPSTR  NhW2M( LPCWSTR pszWideChar )
{
	...
	WideCharToMultiByte(	CP_ACP,	// CODE PAGE: ANSI code page
							...
							-1,
							pszMultiByte,
							(int)length * sizeof(WCHAR),	// ëSäpï∂éöÇÕÅAWCHARÇ≈ÇÕÇPï∂éöÇæÇ™CHARÇ≈ÇÕÇQï∂éöÅÀï∂éöêîÇ™ëùÇ¶ÇÈÅÀWCHARÇÃï∂éöêîÇ≈ÇÕïsè\ï™
							NULL,
							NULL);
	return pszMultiByte;
}
...
LPWSTR NhM2W( LPCSTR  pszMultiByte )
{
	...
	MultiByteToWideChar(	CP_ACP,	// CODE PAGE: ANSI code page
							...
							-1,
							pszWideChar,
							(int)length );	// ëSäpï∂éöÇÕÅACHARÇ≈ÇÕÇQï∂éöÇæÇ™WCHARÇ≈ÇÕÇPï∂éöÅÀï∂éöêîÇ™å∏ÇÈÅÀCHARÇÃï∂éöêîÇ≈è\ï™
	return pszWideChar;
}
